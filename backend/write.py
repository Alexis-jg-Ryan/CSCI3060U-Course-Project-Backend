# write_new_current_accounts function to be implemented here and write_new_master_accounts function to be implemented here
...
def write_new_master_accounts(accounts, file_path):
    """
    Writes the New Master Bank Accounts File consumed by the next backend run.
    Format: NNNNN AAAAAAAAAAAAAAAAAAAA S PPPPPPPP TTTT NP  (45 chars)
    """
    sorted_accounts = sorted(accounts, key=lambda a: a['account_number'].zfill(5))
    ...
    with open(file_path, 'w') as file:
        for acc in sorted_accounts:
            ...
            if not isinstance(total_transactions, int) or total_transactions < 0:
                raise ValueError(f"Invalid transaction count: {total_transactions}")

            # Format fields  (must produce exactly 45 chars to match read_old_bank_accounts)
            acc_num = acc['account_number'].zfill(5)           # 5
            name    = acc['name'].ljust(20)[:20]               # 20
            balance = f"{acc['balance']:08.2f}"                # 8
            txn_cnt = str(total_transactions).zfill(4)         # 4

            # "NNNNN AAAAAAAAAAAAAAAAAAAA S PPPPPPPP TTTT NP"
            line = f"{acc_num} {name} {acc['status']} {balance} {txn_cnt} {plan}"
            assert len(line) == 45, f"Master line length error ({len(line)}): {line!r}"
